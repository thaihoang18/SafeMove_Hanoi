...
export function SearchLocationsView({
  ...
  const filteredLocations = locations.filter((loc) => {
    ...
    const typeMap: Record<string, string[]> = {
      ...
      sports: ["stadium", "court", "track"],
    };
    
    const locTypes = (loc.type || "").toLowerCase();
    const matchesType = typeMap[activeFilter]?.some(t => locTypes.includes(t)) ?? false;
    ...
    <div className="demo-search-container">
      ...
      <div className="search-scrollable-content" ref={scrollContainerRef}>
        <h3 className="section-title">
          Search Results {filteredLocations.length > 0 && `(${filteredLocations.length})`}
        </h3>
        ...
        <div className="result-list">
          {filteredLocations.length > 0 ? (
            filteredLocations.slice(0, visibleCount).map((location) => (
              ...
              >
                {typeof location.image_url === "string" && (
                  <img src={location.image_url} alt={location.name} className="spot-img" />
                )}
          ...
          {filteredLocations.length > 0 && (
            <div className="pagination-footer">
              <span className="pagination-summary">
                Showing {Math.min(visibleCount, filteredLocations.length)} of {filteredLocations.length}
                {pageCount > 1 ? ` · Page ${pageIndex} of ${pageCount}` : ""}
              </span>
              {pageIndex < pageCount && (
